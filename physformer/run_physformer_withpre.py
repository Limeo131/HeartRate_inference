...
from face_preprocess import video2frames
...
def train_test():
    ...
    comparison_dir = os.path.join(args.log, "comparisons")  # NEW: directory for comparison images
    saved_comp = 0                                          # NEW: counter
    COMP_LIMIT = 5
    ...
    for idx, row in df.iterrows():
        ...
        ok = video2frames(
            video_path, save_dir,
            target_frame_count=frame_cnt,
            log_file=log_file,
            use_face_preprocess=True,
            out_size=128,
            comparison_out_dir=comparison_dir,
            save_comparison=save_compare,
            comparison_frame="middle",
            comparison_scale=2,
            preprocess_kwargs={"crop_scale": 4.2, "shift_y": 0.35, "min_box": 128}
        )
        ...
        if not ok:
            continue
        if save_compare:
            saved_comp += 1  # NEW: increment counter
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log', type=str, default="/home/siming/physformer/Inference_Physformer_withpre")
