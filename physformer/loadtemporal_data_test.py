...
class VIPL (Dataset):
    ...
    def __init__(self, info_list, root_dir, transform=None, single_file=None):
        ...
        if single_file is not None:
            self.landmarks_frame = self.landmarks_frame[self.landmarks_frame['file_name'] == single_file].reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        video_path = os.path.join(self.root_dir, str(self.landmarks_frame.loc[idx, 'file_name']))
...
#        total_clips = int((self.landmarks_frame.loc[idx, 'frame_cnt'] - 60) // 160)#+1
        ...
        clip_average_HR  = self.landmarks_frame.loc[idx, 'hr_true']
        ...
        for tt in range(total_clips):
            ...
            for i in range(image_count):
                ...
                if tmp_image is None:
                    tmp_image = prev if prev is not None else np.zeros((128,128,3), np.uint8)
                elif tmp_image.shape[:2] != (128,128):
                    tmp_image = cv2.resize(tmp_image, (132,132), interpolation=cv2.INTER_CUBIC)[2:130,2:130,:]
                prev = tmp_image
...
#         if single_file:
#             # 自动加 .mp4 后缀，跟帧文件夹对齐
#             if not single_file.endswith('.mp4'):
#                 single_file += '.mp4'
#             df = df[df['file_name'].str.strip().apply(lambda x: x if x.endswith('.mp4') else x + '.mp4') == single_file]
...
#                 video_name += '.mp4'
#             frame_dir = os.path.join(root_dir, video_name.replace('.mp4',''))
...
#         frame_dir = os.path.join(self.root_dir, video_name.replace('.mp4',''))
#         frame_list = sorted(os.listdir(frame_dir))
