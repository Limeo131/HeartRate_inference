...
def video2frames(
    ...
    step = max(1e-6, total_frames / float(target_frame_count))
    last_valid = np.zeros((out_size, out_size, 3), np.uint8)
    wrote = 0
    for i in range(target_frame_count):
        frame_idx = int(round(i * step))
        frame_idx = min(max(frame_idx, 0), max(total_frames - 1, 0))  # ★ 夹取
        cap.set(cv2.CAP_PROP_POS_FRAMES, frame_idx)
        ret, frame = cap.read()

        if not ret or frame is None:
            warn = f"[WARN] Could not read frame {frame_idx}"
            print(warn);  log_file and log_file.write(warn + "\n")
            frame_out = last_valid.copy()  # ★ 一定要写出一帧（prev）
        else:
            try:
                frame_out = (preprocess_frame(frame, face_mesh, out_size=out_size, **preprocess_kwargs)
                            if use_face_preprocess else _center_crop_128(frame))
            except Exception as e:
                log_file and log_file.write(f"[WARN] preprocess failed at {frame_idx}: {e}\n")
                frame_out = _center_crop_128(frame)
            last_valid = frame_out

        cv2.imwrite(os.path.join(save_dir, f"image_{i+1:05d}.png"), frame_out)
        wrote += 1
